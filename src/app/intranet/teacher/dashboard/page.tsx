...
const mockAttentionData = [
  ...
  },
  {
    ...
    timestamp: "2024-05-15T09:30:00Z",
    studentId: "S04",
    behavior: "Reading-Book" as const,
...
export default function Dashboard() {
  ...
  const averageAttention = Math.round(
    mockAttentionData.reduce((acc, data) => acc + data.score, 0) /
      mockAttentionData.length
  );
  const lowAttentionAlerts = mockAttentionData.filter(
    (data) => data.score < 60
  ).length;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {[
              { label: "Enfocado", value: 25, color: "bg-green-500" },
              { label: "Levantando la mano",value: 10,color: "bg-blue-500",},
              { label: "Leyendo libro", value: 15, color: "bg-teal-500" },
              { label: "Aburrido", value: 8, color: "bg-orange-500" },
              { label: "Riendo", value: 7, color: "bg-lime-500" },
              { label: "Usando celular", value: 10, color: "bg-red-500" },
              { label: "Pensando", value: 10, color: "bg-yellow-500" },
              { label: "Escribiendo", value: 10, color: "bg-purple-500" },
              { label: "Comida", value: 5, color: "bg-pink-500" },
            ].map((item) => (
              <div
                key={item.label}
          ...
          <div className="space-y-3">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm text-gray-600">
                Carlos Ruiz - Atención óptima con participación positiva
              </span>
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm text-gray-600">
                Juan Pérez - Participación estándar
              </span>
